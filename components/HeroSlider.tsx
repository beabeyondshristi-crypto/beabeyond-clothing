  ...
  return (
    <section className="relative h-[85vh] w-full overflow-hidden border-b border-black">
      {slides.map((slide, index) => {
        const layout = slide.settings?.layout || 'split';
        return (
          <div
            key={slide.id}
          ...
          >
            {layout === 'split' && (
              <div className="grid grid-cols-1 md:grid-cols-2 h-full">
                <div className="flex flex-col justify-center p-8 md:p-16 bg-white h-full border-r border-black">
                  <div className="animate-in slide-in-from-bottom-4 duration-700 fade-in fill-mode-both" style={{ animationDelay: '300ms' }}>
                      <h1 className="text-6xl md:text-8xl font-normal uppercase tracking-tighter leading-[0.85] mb-8 font-serif text-black">
                      {slide.title}
                      </h1>
                      <p className="text-xs md:text-sm max-w-[280px] mb-10 font-light tracking-widest uppercase leading-loose text-gray-500">
                      {slide.subtitle}
                      </p>
                      <Link
                      href={slide.link_url || '/shop'}
                      className="inline-block border border-black px-10 py-3 text-[10px] font-bold uppercase tracking-[0.2em] hover:bg-black hover:text-white transition-colors text-black"
                      >
                      {slide.link_text || 'Shop Collection'}
                      </Link>
                  </div>
                </div>
      ...
      {slides.length > 1 && (
        ...
        const layout = (slide.settings?.layout as string) || 'split';
              ...
              <button
                ...
                className={`w-12 h-0.5 transition-colors duration-300 ${
                  idx === current
                    ...
                    : isSplit ? 'bg-black/20' : 'bg-white/30'
                }`}
              />
            );
          })}
